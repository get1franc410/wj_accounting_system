...
class SmartSearch {
    constructor(inputElement, dataUrl, options = {}) {
        ...
        this.options = {
            ...
            currencySymbol: '₦', // **NEW: Default currency symbol**
            ...options
        };
    ...
    createOptionElement(item, query = '') {
        ...
        // ======================================================================
        // START: Updated financial information block
        // ======================================================================
        const currencySymbol = this.options.currencySymbol;
        let financialHtml = '';
        const receivable = parseFloat(item.receivable_balance);
        const payable = parseFloat(item.payable_balance);
        const limit = parseFloat(item.credit_limit);

        let balancesLine = [];
        // Line 1: Receivable Balance (what they owe us)
        if (receivable > 0) {
            balancesLine.push(`<span style="color: #dc3545;">Receivable: ${currencySymbol}${item.receivable_balance}</span>`);
        }
        // Line 1: Payable Balance (what we owe them)
        if (payable > 0) {
            balancesLine.push(`<span style="color: #0d6efd;">Payable: ${currencySymbol}${item.payable_balance}</span>`);
        }

        // Add the combined balances line if it has content
        if (balancesLine.length > 0) {
            financialHtml += `<div class="small mt-2 fw-bold">${balancesLine.join(' &nbsp;•&nbsp; ')}</div>`;
        }

        // Line 2: Credit Limit on a new line
        if (limit > 0) {
            financialHtml += `<div class="small text-muted">Credit Limit: ${currencySymbol}${item.credit_limit}</div>`;
        }
        
        // Append the entire financial block to the main content
        if (financialHtml) {
            content += financialHtml;
        }
        // ======================================================================
        // END: Updated financial information block
        // ======================================================================
